...
class Command(BaseCommand):
    help = 'Fetch youth data from Airtable API and save to a JSON file for inspection'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Starting to fetch Airtable youth data...'))
        ...
        if not api_key:
            ...
            return
        if not base_id:
            self.stdout.write(self.style.ERROR('Airtable base ID not found in environment variables. Make sure AIRTABLE_YOUTH_BASE_ID is set.'))
            return
        if not table_id:
            self.stdout.write(self.style.ERROR('Airtable table ID not found in environment variables. Make sure AIRTABLE_COMBINED_YOUTH_DATA_TABLE_ID is set.'))
            return
        ...
        try:
            ...
            output_file = os.path.join(output_dir, 'airtable_youth_data.json')
            ...
            if all_records:
                ...
                sample_file = os.path.join(output_dir, 'airtable_youth_sample.json')
